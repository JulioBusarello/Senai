...
function sendMessage() {
    ...
    // Adiciona a mensagem enviada ao chat local (mesmo antes de ser processada pelo servidor)
    if (toUser != "Todos") {
        const chat = document.getElementById('chat')
        chat.innerHTML += `<p><strong>Você</strong> para <strong>${toUser}:</strong> ${message}</p>`;
        chat.scrollTop = chat.scrollHeight; // Garante que o chat se desloque até a mensagem mais recente
    }
